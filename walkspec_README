...
4. Run `python /walkspec/walkspec.py`. By default, the gem5 directory, walkspec directory, and spec directory are as listed here, run-scripts are output to `/walkspec/spec_scripts/` (make sure whatever script directory you use exists), and walkspec also attempts to include `/walkspec/hfi_check.c` in all compilation commands. The default SPEC06 config is `simConfig`, which is just a default gcc config modified to use clang. All of these are customizable. The command should take 3-5 minutes on a decent computer.
